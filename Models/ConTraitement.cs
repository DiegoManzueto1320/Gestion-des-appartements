...
namespace GESTAPP  
{
    public class ConTraitment
    {
        ...
        public string ConnectionString = "Server=localhost; database= bddgestapp; user id=root; password=''; Allow  Zero Datetime=True;Convert Zero Datetime =True;";
        ...
        public string ValeurChamp(string Nomtable, string champRech, string champCle, string valeurCle)
        {
            string resultat = "";
            string requete = $"SELECT {champRech} FROM {Nomtable} WHERE {champCle} = '{valeurCle}' LIMIT 1";
            try
            {
                connex();
                ...
                if (dr.Read())
                {
                    resultat = dr[champRech].ToString();
                }
                dr.Close();
                deconnex();
            }
